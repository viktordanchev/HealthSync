...
const Header = () => {
    ...
    return (
        <header className={`transition-all duration-300 transform flex bg-maincolor ${isFixed ? 'fixed top-0 z-40 h-20 w-4/5 rounded-xl p-5 shadow-xl border-2 border-zinc-500 translate-y-6 sm:translate-y-3 sm:left-3 sm:right-3 sm:w-auto' : 'h-24 w-full p-6 translate-y-0'}`}>
            ...
            <div className={`absolute top-full left-0 right-0 bg-maincolor p-5 rounded-xl mt-2 z-40 border-2 border-zinc-500 hidden md:block sm:block transition-all duration-500 ease-in-out transform ${isMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-[-10px] hidden'} ${isFixed ? '' : 'mx-6'}`}>
